...
def _(pl):
    def remap_id(
        ...
        if old_id_name == new_id_name:
            raise ValueError(
                "old_id_name and new_id_name must have different values"
            )
...
def _(Path, pl, remap_id):
    def divide_by_city(
        ...
        for city in cities:
            ...
            local_nodes, local_edges = remap_id(
                local_nodes, local_edges, "node", "nodeid"
            )
...
@app.cell
def _(DATA_DIR, divide_by_city, edges, nodes, pl, raw_data):
    cities = (
        raw_data.cities.filter(pl.col("pop2025") > 50000)
        .select(pl.col("city"))
        .collect()
        .to_series()
    )
    ...
    divide_by_city(
        nodes.collect().lazy(), edges.collect().lazy(), cities, DATA_DIR
    )
    return
